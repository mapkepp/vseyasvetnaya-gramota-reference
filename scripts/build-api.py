...
bukovy=json.loads((ROOT/"data"/"bukovy.json").read_text("utf-8")); practices=json.loads((ROOT/"data"/"practices.json").read_text("utf-8")); histories=json.loads((ROOT/"data"/"practice"/"practitioner-histories.json").read_text("utf-8")); recovery_pages=json.loads((ROOT/"data"/"recovery"/"historical-six-pages.json").read_text("utf-8")); recovery_candidates=json.loads((ROOT/"data"/"recovery"/"147-candidates.json").read_text("utf-8")); recovery_evidence=json.loads((ROOT/"data"/"recovery"/"secondary-evidence.json").read_text("utf-8")); practical_coverage=json.loads((ROOT/"data"/"practical-coverage.json").read_text("utf-8")); worker_status_path=ROOT/"data"/"research"/"worker-status.json"; implementation_status_path=ROOT/"data"/"research"/"implementation-status.json"; research_status_path=ROOT/"data"/"research"/"autonomous-status.json"; research_queue_path=ROOT/"data"/"research"/"autonomous-queue.json"; research_status_path=worker_status_path if worker_status_path.is_file() else research_status_path; research_status=json.loads(research_status_path.read_text("utf-8")) if research_status_path.is_file() else {"status":"NOT_INITIALIZED"}; research_queue=json.loads(research_queue_path.read_text("utf-8")) if research_queue_path.is_file() else {"tasks":[]}; implementation_status=json.loads(implementation_status_path.read_text("utf-8")) if implementation_status_path.is_file() else {"status":"NOT_INITIALIZED"}
...
BUKOVA_ALIASES={
    ...
    "ра":"ра","ро":"ро","ас":"ас","еръ":"еръ","эсо":"эсо",
...
index["endpoints"]["implementation_status"]="/api/v1/implementation-status.json"
for n,o in [("bukovy.json",bukovy),("practices.json",practices),("practitioner-histories.json",histories),("practical-coverage.json",practical_coverage),("research-status.json",research_status),("research-queue.json",research_queue),("implementation-status.json",implementation_status),("index.json",index),("bukovy-manifest.json",manifest),("recovery.json",recovery)]: (api/n).write_text(json.dumps(o,ensure_ascii=False,indent=2)+"\n","utf-8")
