    ...
    if needed:
        ...
        for rotation_count, total_count, latest, eid, name in candidates[:needed]:
            ...
            additions.append({
                ...
                "generation": gen,
                "queries": [
                    f'"{name}" "Букова" практика',
                    f'"{name}" "Буковы" применение',
                    f'"{name}" "применял" OR "использовал" OR "использовали"',
                    f'"{name}" "нанес" OR "начертал" OR "написал" OR "составил"',
                    f'"{name}" "получил" OR "получилось" OR "результат" OR "эффект"',
                    f'"{name}" "опыт" OR "отзыв" OR "история" OR "случай"',
                    f'"{name}" "практик" OR "практика" OR "мастер"',
                    f'"{name}" "дневник" OR "журнал" OR "отчёт" OR "отчет"',
                    f'"{name}" "форум" OR "обсуждение" OR "комментарии"',
                    f'"{name}" "видео" OR "лекция" OR "семинар" OR "разбор"',
                    f'"{name}" "исследование" OR "наблюдение" OR "эксперимент"',
                    f'"{name}" "до" "после" OR "изменение" OR "динамика"',
                    f'"{name}" "применение" -описание -определение',
                    f'"{name}" "Букова" источник свидетельство практика'
                ],
