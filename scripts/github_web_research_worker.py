...
class P(HTMLParser):
    def __init__(self):
        super().__init__(); self.a=False; self.u=""; self.b=[]; self.r=[]
    def handle_starttag(self,t,a):
        a=dict(a)
        if t=="a" and "result__a" in a.get("class",""):
            self.a=True; self.u=a.get("href",""); self.b=[]
...
def main():
    ...
    for q in task.get("queries",[]):
        try:
            ...
            p=P(); p.feed(body)
            for title,url in p.r[:8]:
                found.setdefault(url,{"title":title,"queries":[]})["queries"].append(q)
        except Exception:
            pass
    findings=[{
        "claim":"Discovered public-web source; requires verification before use.",
        "sources":[{"url":u,"title":m["title"]}],
        "evidence_type":"web_discovery",
        "confidence":"UNSPECIFIED",
        "conflicts":[],
        "discovery_queries":m["queries"]
    } for u,m in list(found.items())[:8]]
    json.dump({
        "schema_version":"1.0","task_id":task["task_id"],"bukova":task.get("bukova"),
        "status":"PASS" if findings else "FAIL","evidence":"MEASURED",
        "generated_at":datetime.now(timezone.utc).isoformat(),
        "queries":task.get("queries",[]),"search_engine":"github-actions-public-web",
        "findings":findings
